<template>
  <header 
    class="fixed top-0 left-0 right-0 z-50 px-7 py-4 bg-dark/80 backdrop-blur supports-[backdrop-filter]:bg-dark/50"
    role="banner"
  >
    <nav class="max-w-3xl mx-auto flex justify-between items-center">
      <!-- Home link -->
      <NuxtLink to="/" class="text-neutral-200 font-bold text-xl hover:text-neutral-100 transition-colors">
        Dmxsan
      </NuxtLink>
      ...
      <div class="hidden md:flex items-center space-x-4">
        <ul class="flex space-x-4 text-sm">
          <li v-for="item in navItems" :key="item.path">
            <NuxtLink 
              :to="item.path"
              class="hover:text-neutral-200 transition-colors outline-none focus:ring-2 focus:ring-blue-500 rounded px-2 py-1"
              :class="{ 
      ...
      <div 
        v-if="isMenuOpen"
        class="md:hidden fixed inset-x-0 top-[73px] p-4 bg-dark/95 backdrop-blur"
      >
        <ul class="flex flex-col space-y-4">
          <li v-for="item in navItems" :key="item.path">
            <NuxtLink 
              :to="item.path"
              class="block py-2 hover:text-neutral-200 transition-colors"
              :class="{ 
                'text-neutral-200': isActive(item.path),
                'text-neutral-400': !isActive(item.path)
              }"
